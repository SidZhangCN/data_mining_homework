...
for paper_id in train_paper_list:
    ...
    assert len(persons) == (
        len(list(author_data[paper_author_name].keys())) - 1)
...
def clean_name(name):
    if name is None:
        return ""
    x = [k.strip() for k in name.lower().strip().replace(
...
def delete_main_name(author_list, name):
    ...
    for author in author_list_lower:
        ...
        score = distance.get_jaro_distance(
            name, author, winkler=True, scaling=0.3)
...
def process_feature(pos_ins, paper_coauthors):
    ...
    for doc in doc_list:
        ...
        paper_authors = random.sample(
            paper_authors, min(50, paper_authors_len))
    ...
    if ((len(author_keys) == 0) or (len(paper_coauthors) == 0)):
        ...
        co_coauthors_ratio_for_paper = round(
            coauthor_len / len(paper_coauthors), 6)
        co_coauthors_ratio_for_author = round(
            coauthor_len / len(author_keys), 6)

        coauthor_count = 0
        for coauthor_name in co_coauthors:
            coauthor_count += candidate_authors_int[coauthor_name]

        co_coauthors_ratio_for_author_count = round(
            coauthor_count / total_author_count, 6)
        ...
        feature_list.extend([coauthor_len, co_coauthors_ratio_for_paper,
                             co_coauthors_ratio_for_author, coauthor_count, co_coauthors_ratio_for_author_count])
...
clf = SVC(probability=True, C=0.5, kernel='linear',
...
def process_test_feature(pair, new_test_author_data, test_pubs_dict, paper_coauthors):
    ...
    for doc in doc_list:
        ...
        paper_authors_len = len(paper_authors)
        paper_authors = random.sample(
            paper_authors, min(100, paper_authors_len))
...
#     paper_authors = random.sample(paper_authors, min(50, paper_authors_len))
    ...
    paper_name = '_'.join(clean_name(
        paper_authors[author_index]['name']).split())
    if(new_test_author_data.get(paper_name) != None):
        ...
        for candidate_author in candidate_author_list:
            ...
            paper2features[paper_id].append(process_test_feature(
                pair, new_test_author_data, test_pubs_dict, paper_coauthors))
        count += 1
print(count)
